  ...
  bdd =>
    bdd
      ...
      .and.oneOf([
        ...
        ['an iterable', ({ reducer }) => ({ reducer, targets: [arrayAsIterable([1, 2, 3]), arrayAsIterable([4])] as const })]
      ])
      ...
      .when('rendered with [4]', ({ reducer, targets }, result) => {
        result.rerender({ reducer, target: targets[1] });

        return result;
      })
      .then('result should return 4', (_, result) => expect(result.result).toHaveProperty('current', 4))
